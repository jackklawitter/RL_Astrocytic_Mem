...
if __name__ == "__main__":
    ...
    obs = env.reset()  # Initialize the environment first!
    
    while True:
        action = int(input("Left (1) or Right (2): ")) - 1  # Subtract 1 to match the action mapping
        
        if action < 0:
            break 

        obs, reward, done, time_step = env.step(action)
        
        if done:
            print("Episode complete! Starting new episode...")
            obs = env.reset()
